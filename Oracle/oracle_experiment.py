...
def get_oracle_fmeta(config, path=None):
    set_num = 4
    channels = 64
    kernel_size = 3

    if config['num_classes'] == 20:
        if config['k_shot'] == 1 or config['k_shot'] == 3:
    ...
    elif config['num_classes'] == 200:
        if config['k_shot'] == 1:
            parall_num = config['meta_batch_size'] // 1
        elif config['k_shot'] == 3 or config['k_shot'] == 5:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--meta_batch_size", "-mbs", help="number of tasks sampled per meta-update", default=32,
                        type=int)
    parser.add_argument("--img_num", default=25, type=int)
    parser.add_argument("--fd_shot", default=5, type=int)
    ...
    parser.add_argument("--k_shot", "-ks",
                        help="number of examples used for inner gradient update (K for k_shot learning).", default=5,
                        type=int)
    parser.add_argument("--k_query", "-kq", help="number of examples used for inner test (K for k_query).", default=5,
                        type=int)
    ...
    parser.add_argument("--model_name", "-mn", help="The name of model used in the task.",
                        default='fuzzymeta4')
    parser.add_argument("--m_gpu", help="multi GPU.",
                        default=True)
    parser.add_argument("--data_source", help="data_source.", default='origin')
    config = parser.parse_args().__dict__
    # print(config.keys())
    if config['data_source'] != 'origin':
        config['fd_shot'] = config['k_shot']
    # channel = 256

    tasks_file = '{}W_{}S_{}Q_{}'.format(str(config['num_classes']), str(config['k_shot']), str(config['k_query']), config['data_source'])

    # tasks_file = '{}W_{}S_{}Q_{}_{}'.format(str(config['num_classes']), str(config['k_shot']), str(config['k_query']),
    #                                      config['data_source'], channel)


    #
    # tasks_file = str(config['num_classes']) + 'W_' + \
    #                       str(config['k_shot']) + 'S_' + str(config['k_query']) + 'Q'
    ...
    if config['train']:
        ...
        trainset = Oracle(use_set, resume_itr, config)

        # print(config['img_num'])
        # print(config['fd_shot'])
        trainset.get_tasks()
        trainset = torch.utils.data.DataLoader(trainset, batch_size=config['meta_batch_size'],
                                    shuffle=False, num_workers=8)
        config['img_num'] = 20
        valset = Oracle('test', resume_itr, config)
        valset.get_tasks()
        ...
        config['train'] = False
        valset = Oracle('test', config=config)
        valset.get_tasks()
        valset = torch.utils.data.DataLoader(valset, batch_size=config['meta_batch_size'],
                                             shuffle=False, num_workers=8)
        metanet.eval()
        ...
        testset = Oracle('test', config=config) if config['test_or_val'] else Oracle('val', config=config)
        testset.get_tasks()
